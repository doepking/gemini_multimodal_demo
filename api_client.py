...
async def create_session(user: Dict[str, Any], session_id: str):
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        response = await client.post(
            f"{API_URL}/apps/gemini_adk_demo/users/{user['id']}/sessions/{session_id}",
            json=session_state,
            headers=headers,
        )
    response.raise_for_status()
...
async def get_chat_response(user_id: int, user_email: str, user_name: str, session_id: str, user_prompt: str = None, audio_file_path: str = None) -> Dict[str, Any]:
    """Gets a chat response from the backend."""
    logger.info(f"api_client.get_chat_response: user_id: {user_id}, session_id: {session_id}")
...
async def get_recent_metrics(user_email: str, limit: int = 30):
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        response = await client.get(f"{API_URL}/metrics/user/{user_email}?limit={limit}", headers=headers)
...
async def get_subscription_status(user_email: str):
    ...
    async with httpx.AsyncClient(timeout=5) as client:
        response = await client.get(f"{API_URL}/newsletter/preferences/{user_email}", headers=headers)
    if response.status_code == 404:
...
async def unsubscribe_from_newsletter(user_email: str):
    ...
    if not secret_key:
        raise ValueError("SUBSCRIPTION_SECRET_KEY is not set in the environment.")
